...
Please Note this is an automated email, <b>Do Not Reply</b>');
      ...
      case 'task_delete':
        ...
        tep_redirect(tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'page=' . $HTTP_GET_VARS['page']));
        break;
      case 'project_define':
	...
	//else{
	 ...
	 if ($project_id<=0) tep_redirect(tep_href_link(FILENAME_ADMIN_TASKS_ALL));
	 ...
	 tep_redirect(tep_href_link(FILENAME_ADMIN_TASKS_ALL,'action=define_project&projPath=' . $project_id));
      case 'project_delete':
        ...
        tep_redirect(tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=projects'));
        break;        
      case 'project_edit':
        ...
        if (($admin_projects_name == '' || NULL) || (strlen($admin_projects_name) <= 5) ) {
          tep_redirect(tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $HTTP_GET_VARS[projID] . '&gName=false&action=action=edit_project'));
        } else {
          ...
          if ($check_duplicate > 0){
            tep_redirect(tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $HTTP_GET_VARS['projID'] . '&gName=used&action=edit_project'));
          } else {
            ...
            tep_redirect(tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $admin_projects_id));
          }
        }
        break;              
      case 'project_new':
        ...
        if (($admin_projects_name == '' || NULL) || (strlen($admin_projects_name) <= 5) ) {
          tep_redirect(tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $HTTP_GET_VARS[projID] . '&gName=false&action=new_project'));
        } else {
          ...
          $check_duplicate = tep_db_num_rows($check_projects_name_query);
          if ($check_duplicate > 0){
            tep_redirect(tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $HTTP_GET_VARS['projID'] . '&gName=used&action=new_project'));
          } else {
      ...
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td class="smallText" align="right"><?php //echo tep_draw_form('status', FILENAME_ADMIN_TASKS_ALL, '', 'get'); ?>
...
<table border="0" width="100%" cellspacing="0" cellpadding="2" class="sort-table" id="table-1">
	   ...
	   <td class="dataTableHeadingContent">&nbsp; </td>
			    ...
			    <td class="dataTableHeadingContent" title="CaseInsensitiveString" width="121">Task Name</td>
                ...
                <td class="dataTableHeadingContent" align="center" title="CaseInsensitiveString"> Priority</td>
				<td class="dataTableHeadingContent" align="center" title="CaseInsensitiveString">Assigned To:</td>
				<td class="dataTableHeadingContent" align="center" title="Number">Status</td>
				<td width="72" align="center" valign="middle" class="dataTableHeadingContent" title="Date">Due Date</td>
                <td class="dataTableHeadingContent" align="right">Action</td>
              </tr>
			  </THEAD>
<?php
  $db_admin_query_raw = "select * from " . TABLE_ADMIN_TASKS . " order by admin_task_id";
  ...
  while ($admin = tep_db_fetch_array($db_admin_query)) {
   ...
   if ( (is_object($tInfo)) && ($admin['admin_task_id'] == $tInfo->admin_task_id) ) {
      echo '<tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'page=' . $HTTP_GET_VARS['page'] . '&tID=' . $admin['admin_task_id'] . '&action=edit_task') . '\'">' . "\n";
    } else {
      echo '<tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'page=' . $HTTP_GET_VARS['page'] . '&tID=' . $admin['admin_task_id']) . '\'">' . "\n";
    }
?>
<td valign="top" class="dataTableContent"><div align="center"><a href="admin_tasks_all.php?tID=<?php echo $admin['admin_task_id']?>&action=edit_task"><IMG SRC="images/icons/image_edit.gif" border="0"></a></div></td>
                <td width="121" align="center" class="dataTableContent"><a href="admin_tasks_all.php?tID=<?php echo $admin['admin_task_id']?>&action=edit_task"><?php echo $admin['admin_task_name']; ?></a></td>
	...
	<td align="center" valign="top" class="dataTableContent"><?php //echo $admin['admin_id'];
...
?></div> </td>
                ...
                <td class="dataTableContent" align="right"><?php if ( (is_object($tInfo)) && ($admin['admin_id'] == $tInfo->admin_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'page=' . $HTTP_GET_VARS['page'] . '&tID=' . $admin['admin_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
...
var st1 = new SortableTable(document.getElementById("table-1"),
	["None", "CaseInsensitiveString", "CaseInsensitiveString", "CaseInsesitiveString", "CaseInsensitiveString", "CaseInsensitiveString", "Number", "Date", "None"]);
              ...
              <tr>
                <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td class="smallText" valign="top" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=projects') . '">' . tep_image_button('button_admin_projects.gif', 'Projects List') . '</a>'; echo ' <a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'page=' . $HTTP_GET_VARS['page'] . '&tID=' . $tInfo->admin__task_id . '&action=new_task') . '">' . tep_image_button('button_admin_task.gif', 'New Task') . '</a>'; ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
            </table>
  ...
  switch ($HTTP_GET_VARS['action']) {  
    case 'new_task': 
      ...
      if ($HTTP_GET_VARS['orgin']!='' && (int)$HTTP_GET_VARS['projID']!=0){
         $contents = array('form' => tep_draw_form('newtask', FILENAME_ADMIN_TASKS_ALL, 'action=task_new&orgin=' . $HTTP_GET_VARS['orgin'] . '&projID=' . $HTTP_GET_VARS["projID"], 'post', 'enctype="multipart/form-data"')); }
      else {
         $contents = array('form' => tep_draw_form('newtask', FILENAME_ADMIN_TASKS_ALL, 'action=task_new&page=' . $page . 'tID=' . $HTTP_GET_VARS['tID'], 'post', 'enctype="multipart/form-data"')); }
	  ...
	  //Added for Assignees
	   ...
	   $priority_array = array(array('id' => '0', 'text' => TEXT_NONE));
		...
		$contents[] = array('text' => '<br>&nbsp;Task Status:<br>&nbsp;' . tep_draw_pull_down_menu('admin_task_status', $status_array, '0')); 
		  	  //END
			   $contents[] = array('text' => '<br>&nbsp;Task Due Date:<br>&nbsp;' . tep_draw_input_field('admin_task_due')); 
			  $contents[] = array('text' =>'<a href="javascript:showCal(\'Calendar1\')">Select Date</a>');
              $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_insert.gif', IMAGE_INSERT, 'onClick="validateForm();return document.returnValue"') . ' <a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'page=' . $HTTP_GET_VARS['page'] . '&tID=' . $HTTP_GET_VARS['tID']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      }
      break;
    case 'edit_task': 
...
tep_draw_textarea_field('admin_task_description', 'soft', '15', '15', $tInfo->admin_task_description));
    ...
    case 'del_task': 
      ...
      $contents = array('form' => tep_draw_form('edit', FILENAME_ADMIN_TASKS_ALL, 'action=task_delete&page=' . $page . '&tID=' . $admin['admin_task_id'], 'post', 'enctype="multipart/form-data"')); 
      $contents[] = array('text' => tep_draw_hidden_field('admin_task_id', $tInfo->admin_task_id));
      $contents[] = array('align' => 'center', 'text' =>  sprintf('Are you certain you wish to delete the current task'. $tInfo->admin_task_name .'?'));    
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'page=' . $HTTP_GET_VARS['page'] . '&tID=' . $HTTP_GET_VARS['tID']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');    
      break;
    case 'new_project':
      $heading[] = array('text' => '<b>Adding a New Project</b>');

      $contents = array('form' => tep_draw_form('new_project', FILENAME_ADMIN_TASKS_ALL, 'action=project_new&projID=' . $gInfo->admin_projects_id, 'post', 'enctype="multipart/form-data"')); 
      ...
      $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $gInfo->admin_projects_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a> ' . tep_image_submit('button_next.gif', IMAGE_NEXT) );    
      break;
    case 'edit_project': 
      $heading[] = array('text' => '<b>Editing Project:&nbsp;'. $gInfo->admin_projects_name .'</b>');

      $contents = array('form' => tep_draw_form('edit_project', FILENAME_ADMIN_TASKS_ALL, 'action=project_edit&projID=' . $HTTP_GET_VARS['projID'], 'post', 'enctype="multipart/form-data"')); 
      if ($HTTP_GET_VARS['gName'] == 'false') {
        $contents[] = array('text' => TEXT_INFO_GROUPS_NAME_FALSE . '<br>&nbsp;');
      } elseif ($HTTP_GET_VARS['gName'] == 'used') {
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.gif', IMAGE_SAVE) . ' <a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $gInfo->admin_projects_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');    
      break;
    case 'del_project': 
      $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_DELETE_GROUPS . '</b>');

      $contents = array('form' => tep_draw_form('delete_project', FILENAME_ADMIN_TASKS_ALL, 'action=project_delete&projID=' . $gInfo->admin_projects_id, 'post', 'enctype="multipart/form-data"')); 
      if ($gInfo->admin_projects_id == 1) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $HTTP_GET_VARS['projID']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a><br>&nbsp;');
      } else {
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $HTTP_GET_VARS['projID']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a><br>&nbsp;');    
      }
      break;
    case 'define_project':      
      ...
      if ($HTTP_GET_VARS['projPath'] == 1) {
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $HTTP_GET_VARS['projPath']) . '">' . tep_image_button('button_back.gif', IMAGE_CANCEL) . '</a><br>');
      } else {
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'action=project_default&projPath=' . $HTTP_GET_VARS['projPath']) . '">' . tep_image_button('button_set_default.gif', IMAGE_RIGHTS_SET_DEFAULT) . '</a><br>');
      }
      break;
    case 'show_project': 
      ...
      $contents = array('form' => tep_draw_form('show_project', FILENAME_ADMIN_TASKS_ALL, 'action=show_project&projID=projects', 'post', 'enctype="multipart/form-data"')); 
      $contents[] = array('text' => $define_files['admin_files_name'] . tep_draw_input_field('level_edit', $checkEmail)); 
      //$contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_insert.gif', IMAGE_INSERT) . ' <a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $gInfo->admin_projects_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');    
      break;
    default:
      if (is_object($tInfo)) {
        $heading[] = array('text' => '<b>&nbsp;Task Information:</b>');
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'page=' . $HTTP_GET_VARS['page'] . '&tID=' . $tInfo->admin_task_id . '&action=edit_task') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'page=' . $HTTP_GET_VARS['page'] . '&tID=' . $tInfo->admin_task_id . '&action=del_task') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a><br>&nbsp;');
        ...
        $display_text =' <a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $gInfo->admin_projects_id . '&action=edit_project') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_ADMIN_TASKS_ALL, 'projID=' . $gInfo->admin_projects_id . '&action=del_project') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>';
